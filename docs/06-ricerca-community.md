...
| R13 | [hilalitvak/inkalign](https://github.com/hilalitvak/inkalign) @ `c11177a` (run del 25 agosto 2026): sweep di finestre Z e orientamenti di `ink_9um` su una ROI del segmento w025, con un punteggio **senza etichette** di "somiglianza a testo" (`line_score`, periodicità dei tratti via FFT); migliore dichiarata: offset −4. | Trovato dal socio in E03 S1 (L1). Non misura pixel held-out né AUROC: non è una curva di tolleranza, ma è la sola strada praticabile su un rotolo non letto. Candidato naturale per una prova E04: il verso migliore stimato senza label coincide con quello misurato da noi con le label sui due segmenti di sviluppo? Non replicato. |
| R14 | [flummoxjr/measure-before-you-hunt](https://github.com/flummoxjr/measure-before-you-hunt) @ `e508085`, `hunt/depth_offset_plan.md` (17 agosto 2026; copia identica in `gp13-ink-detectability`): curva AUC forward per offset da −6 a +5 voxel di `ink_9um` seed 42 sul controllo **w035** (segmento di training), da 0,889 (−6) a 0,966 (0) a 0,871 (+5); conclusione dell'autore: "the depth-offset hypothesis is refuted for offsets within ±6 voxels". | Trovato dal socio in E03 S1 (L1): la misura numerica pubblicata più vicina alla nostra. Differisce per popolazione (pixel di un segmento di training, volume nativo a 28 slice), seed e segmenti (uno e uno) e criterio di lettura: con il criterio dell'autore l'ipotesi è "smentita", con la regola preregistrata di E03 (perdita ≥ 0,05) la stessa curva perde 0,095 a +5. La nostra curva sui pixel di training perde al massimo 0,027: i due controlli di training non coincidono e non ne traiamo conclusioni causali. Da citare in ogni presentazione di E03: la nostra novità è la misura su pixel held-out, con due seed, due segmenti e lettura per verso, non l'idea dello sweep. Non replicato. |
