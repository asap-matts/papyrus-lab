...
2. **E01**: [piano](plans/2026-09-06-e01-ripetizione-socio.md) e [prompt per il Codex del socio](plans/e01-prompt-codex.md) pronti. Il socio ripete E00 **dal suo account Kaggle** (i notebook girano su Kaggle; il Mac è solo il telecomando), con Codex come esecutore e lui che approva i run GPU; lavora sul branch `e01-socio`, che Matteo fonde dopo la revisione. Stato al 6 settembre 2026: notebook generati per l'account Kaggle `micheleghisa`, branch `e01-socio` creato, invito GitHub a `micheleghisa` inviato (permesso write, da accettare), script di avvio per Mac pronto; al socio restano i login (`gh`, `kaggle`) e i due "vai" GPU. Non è cieco per costruzione (esito calcolato, non giudicato) e lo dichiara.
