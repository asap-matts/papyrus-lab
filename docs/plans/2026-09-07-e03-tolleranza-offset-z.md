...
| 4 | P1 | Il manifest attestava senza verificare: `partner_start` preso dalla punta del branch (f579f5b invece di 8cfdc35), S1–S3 scritti a mano nel codice, `curve_summary` copiato da `curve.json`, sigillo dichiarato senza scansione, `opened_in: E05` fuori perimetro | accettato | `e03_manifest.py`: commit di partenza risolto dal soggetto scritto nel piano del socio e verificato; file consegnati confrontati con l'elenco atteso; S2 letto dal rapporto e confrontato con `datasets.json` e con l'impronta dello script; S3 letto dal rapporto e confrontato cella per cella (28 AUROC, 6 tolleranze, H1, H2, anomalia, controlli); curva ricalcolata dai 28 report e confrontata con `curve.json`; 61 JSON scanditi per il sigillo; `opened_in` rimosso. Prima versione con controlli in parte sintattici: resa strutturale al secondo giro (sotto) |

- **Secondo giro** (`gpt-5.6-sol`, `--base 1cb603e`, job `review-mtrqeykx-jo3gtp`): numeri della scheda confermati corretti; **NO-SHIP, 3 finding (1 alto, 2 medi), accettati 3.**

| # | Sev. | Finding (sintesi) | Esito | Correzione |
|---|---|---|---|---|
| 1 | alto | Il parser dei rapporti del socio poteva certificare contenuti incompleti o non numerici: sette copie della riga k=0 davano 28 celle e scarto 0; `nan` passava; H1 confrontava solo il conteggio; tolleranza e H2 non richiedevano chiavi uniche; i controlli contavano due frasi ovunque; S1 accettava un verdetto vuoto; S2 contava due `True` sciolti | accettato | `check_s1/check_s2/check_s3` come funzioni pure su testo: tabelle lette per intestazione, insiemi esatti di offset/chiavi senza duplicati, `math.isfinite` su ogni numero, H1 confrontata come insieme (segmento, seed, offset, Δ), verdetti dei controlli letti nella propria sezione, verdetto S1 in un elenco chiuso, uguaglianza slice legata alle due frasi attese. 16 test negativi (`tests/test_e03_manifest.py`): riga duplicata, offset mancante, `nan`/`inf`, colonne permutate, stessa numerosità H1 con violazione diversa, riga H2 o di tolleranza duplicata, verdetto fuori sezione, anomalia, S1 vuoto, S2 alterato, curva alterata, modo sul sigillato |
