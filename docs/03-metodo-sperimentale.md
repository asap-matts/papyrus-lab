...
2. Distinguere dati di sviluppo e dati finali di verifica. Conservare separatamente regioni realmente escluse dal training di tutti i checkpoint valutati, controllando i manifest ufficiali. Attenzione a due trappole verificate nel dataset `ink_9um`: lo stesso segmento fisico può comparire in più rappresentazioni (w035 è nel training sia come volume 2,4 µm ridotto sia come nativo 9,362 µm), e i nomi `pherc0139-wNNN` delle label non seguono la numerazione pubblica dei segmenti (`pherc0139-w016` è il segmento pubblico `w029`). Se non riusciamo a dimostrare l'esclusione, chiamare la prova controllo operativo e non benchmark indipendente.
